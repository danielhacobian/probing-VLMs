...
This directory records the frozen straightening-OFF and straightening-ON
checkpoints used by the standalone UMaze layerwise probe notebook. Binary parts
live in this repository's `probe-checkpoints-v1` GitHub Release.

Run `restore_checkpoints.sh` from the repository root to download, concatenate,
and verify both checkpoints. The notebook performs the same restoration.
